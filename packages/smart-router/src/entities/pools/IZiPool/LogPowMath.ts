...
export function getSqrtPrice(point: number): BigNumber {
  ...
  return JSBI.greaterThan(JSBI.remainder(ratio, Q32), ZERO)
    ? BigNumber.from(JSBI.add(JSBI.divide(ratio, Q32), ONE).toString())
    : BigNumber.from(JSBI.divide(ratio, Q32).toString())
}

export function getLogSqrtPriceFloor(_sqrtRatioX96: BigNumber): number {
  ...
  return pointLow === pointHigh
    ? pointLow
    : JSBI.lessThanOrEqual(JSBI.BigInt(getSqrtPrice(pointHigh).toString()), sqrtRatioX96)
      ? pointHigh
      : pointLow
}
