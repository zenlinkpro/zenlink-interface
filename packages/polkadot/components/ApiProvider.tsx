...
export const PolkadotApiContext = createContext<ApiContext | undefined>(
  undefined,
)

class Manage {
  loading: boolean
  subscription?: Promise<Unsubcall | void>
  constructor() {
    this.loading = false
  }
...
export const PolkadotApiProvider = ({ chains, children, store }: Props) => {
  ...
  const [states, setStates] = useState<ApiContext['states']>(
    chains.reduce(
      (states, chain) => ({
        ...states,
        [chain.id]: { hasInjectedAccounts: false, isApiReady: false },
      }),
      {},
    ),
  )
              ...
              if (api) {
                ...
                api.on('ready', () => {
                  loadOnReady(api, injectedPromise, store, types)
                    .then(state =>
                      setStates(prev => ({ ...prev, [chain.id]: state })),
                    )
                    .catch(onError)
                })
              }
